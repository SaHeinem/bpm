...
import { Search, Users, UtensilsCrossed, ArrowRightLeft, Trash2, AlertTriangle, Download } from "lucide-react"
...
export function AssignmentsView() {
  const { assignments, isLoading: assignmentsLoading, assignMutation, unassignMutation, clearAssignmentsMutation } =
    useAssignments()
  ...
  const [activeTab, setActiveTab] = useState<"people" | "restaurants">("people")
  ...
  const nonCaptainParticipants = useMemo(
    () => eligibleParticipants.filter((participant) => !participant.is_table_captain),
  ...
  const unassignedParticipants = useMemo(
    () => nonCaptainParticipants.filter((participant) => !assignmentsByParticipant[participant.id]),
  ...
  const filteredPeople = useMemo(() => {
    ...
    return eligibleParticipants.filter((participant) => {
      ...
      const restaurantName = assignmentRestaurantId
        ? restaurants.find((restaurant) => restaurant.id === assignmentRestaurantId)?.name ?? ""
        : ""
      return (
  ...
  }, [eligibleParticipants, searchQuery, assignmentsByParticipant, restaurants])
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    return restaurants.filter(
      (restaurant) =>
        restaurant.name.toLowerCase().includes(term) || restaurant.address.toLowerCase().includes(term),
    )
  ...
  const handleManualAssign = async (participantId: string, restaurantId: string | null) => {
    if (isFinalized) {
      toast({
        title: "Event finalized",
        description: "Assignments are locked. Unfinalize the event to make changes.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      if (!restaurantId) {
        ...
        await assignMutation.mutateAsync({ participant_id: participantId, restaurant_id: restaurantId })
  ...
  const handleAddToRestaurant = async (restaurantId: string, participantId: string) => {
    if (!participantId) return
    if (isFinalized) {
      toast({
        title: "Event finalized",
        description: "Assignments are locked. Unfinalize the event to add participants.",
        variant: "destructive",
      })
      return
    }
  ...
  const handleClearAssignments = async () => {
    ...
    try {
      ...
      await activityLogger.mutateAsync({
        event_type: "assignment",
        description: "Cleared assignments from assignments view",
        actor: null,
        metadata: { previousAssignments: assignments.length },
      })
      toast({
  ...
  const dataLoading = participantsLoading || restaurantsLoading || assignmentsLoading

  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-bold tracking-tight text-foreground">Assignments</h2>
        <p className="text-muted-foreground">Search, reassign, or clear participant restaurant placements.</p>
      </div>
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-3 pt-6">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" className="gap-2 bg-transparent" onClick={handleExport} disabled={!assignments.length}>
        ...
        <TabsContent value="people" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Assign or move individuals between restaurants.</CardDescription>
            </CardHeader>
            <CardContent>
              {dataLoading ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>Try adjusting your filters or import additional participants.</AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-3">
                  {filteredPeople.map((participant) => {
                    ...
                    const restaurant = assignmentRestaurantId
                      ? restaurants.find((item) => item.id === assignmentRestaurantId)
                      : undefined
                      ...
                      >
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <p className="font-medium text-foreground">{participant.attendee_name}</p>
                            {isCaptain && (
                          ...
                          <p className="text-xs text-muted-foreground">{participant.attendee_email}</p>
                        </div>
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => handleManualAssign(participant.id, value || null)}
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              {restaurants.map((restaurantOption) => (
                                <SelectItem key={restaurantOption.id} value={restaurantOption.id}>
        ...
        <TabsContent value="restaurants" className="space-y-4">
          {dataLoading ? (
            ...
            filteredRestaurants.map((restaurant) => {
              const assignedParticipantIds = assignmentsByRestaurant[restaurant.id] ?? []
                ...
                <Card key={restaurant.id}>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      {assignedParticipants.length === 0 ? (
                        ...
                        assignedParticipants.map((participant) => (
                          ...
                          >
                            <div>
                              <p className="text-sm font-medium text-foreground">{participant.attendee_name}</p>
                              <p className="text-xs text-muted-foreground">{participant.attendee_email}</p>
                            </div>
                            <Button
                              variant="ghost"
                              ...
                              onClick={() => handleManualAssign(participant.id, null)}
                              disabled={isBusy || isFinalized}
                            >
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Select
                        value={pendingAdd[restaurant.id] ?? ""}
                        onValueChange={(value) => {
                          setPendingAdd((prev) => ({ ...prev, [restaurant.id]: value }))
                          if (value) {
                            void handleAddToRestaurant(restaurant.id, value)
                          }
                        }}
                        disabled={isBusy || unassignedParticipants.length === 0 || isFinalized}
                      >
                        <SelectTrigger className="w-[260px]">
                          <SelectValue placeholder={unassignedParticipants.length ? "Add participant…" : "No unassigned participants"} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="">Select participant</SelectItem>
